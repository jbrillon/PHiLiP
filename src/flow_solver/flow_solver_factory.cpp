...
#include "flow_solver_cases/circular_shell_flow.h"
                   ...
                   const dealii::ParameterHandler &parameter_handler_input)
{
    ...
    if (flow_type == FlowCaseEnum::taylor_green_vortex){
        ...
        if constexpr (dim==2 && nstate==dim+2) {
            std::shared_ptr<FlowSolverCaseBase<dim, nstate>> flow_solver_case = std::make_shared<CircularShellFlow<dim, nstate>>(parameters_input);
            return std::make_unique<FlowSolver<dim, nstate>>(parameters_input, flow_solver_case, parameter_handler_input);
        }
    } else {
