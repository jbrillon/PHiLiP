...
DipoleWallCollisionUnsteadyQuantityCheck<dim, nstate>::DipoleWallCollisionUnsteadyQuantityCheck(
...
template <int dim, int nstate>
int DipoleWallCollisionUnsteadyQuantityCheck<dim, nstate>::run_test() const
{
    ...
    std::unique_ptr<FlowSolver::DipoleWallCollision<dim, nstate>> flow_solver_case = std::make_unique<FlowSolver::DipoleWallCollision<dim,nstate>>(this->all_parameters);
